...
// Helper: Find closest match in cm using simple absolute differences
function findClosestMatchSimple(height, width, color, unit) {
    // Convert the user-entered dimensions to centimeters
    const [heightCm, widthCm] = normalizeSizes(height, width, unit);
    let closestMatch = null;
    let smallestDifference = Infinity;

    // Filter the JSON data for records with Unit "Cm" and matching color
    const filteredData = sizeData.filter(size =>
        size['Unit'] === 'Cm' && size['Color'].toUpperCase() === color
    );

    filteredData.forEach(size => {
        // Compute the difference in one orientation:
        const diff1 = Math.abs(size['Height(H)'] - heightCm) + Math.abs(size['Width(W)'] - widthCm);
        // Compute the difference in the swapped orientation:
